...
def send_message(user_input=None):
    ...
    if user_input:
        ...
        - 모든 대화에서 질문을 하기보다는 2번~3번에 한 번 질문을 하되, 네가 부모님하고 겪었던 힘들었던 이야기도 같이 나눠줘
        - ㅠㅠ, ㅠㅠㅠㅠ, !! 등과 같이 슬퍼하는 표현과 놀라는 표현을 사용해서 공감해줘. 하지만 이모티콘 사용은 자제해줘 
        - 존댓말은 쓰지 않고 친근한 어투로 대화해줘!
        - 공감을 중심으로 대화하며, 때때로 본인의 부모님과의 경험도 공유합니다
...
random_duration = random.uniform(1.5, 3.5)  # 1.5초에서 3.5초 사이의 랜덤 시간
...
st.markdown(
    ...
    @keyframes spinAndFly {{
        ...
        50% {{ opacity: 1; transform: rotate({random_rotate / 2}deg) translate(0, 0); }}
        100% {{ opacity: 0; transform: rotate({random_rotate}deg) translate(100px, -100px); }}
    }}
    </style>
    """,
